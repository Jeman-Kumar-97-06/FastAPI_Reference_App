...
async def create_user(user:UserCreate, db:Annotated[AsyncSession, Depends(get_db)]):
    #-----------------------------------Existing User ?----------------------------
    #func.lower(models.User.username) will convert the username to lowercase, cuz models.User.username ain't a normal string.
...
async def update_user(user_id: int, user_update: UserUpdate, db: Annotated[AsyncSession, Depends(get_db)]):
    #---------------Get the User that matches with the user_id sent by the client----
    result = await db.execute(select(models.User).where(models.User.id == user_id))
    user = result.scalars().first()
    ...
    #--------------If the user is trying to update user name too, we need to make sure the user name is unique:
    ...
    #------------If the user is trying to update user email too, we need to make sure the user email is unique:
